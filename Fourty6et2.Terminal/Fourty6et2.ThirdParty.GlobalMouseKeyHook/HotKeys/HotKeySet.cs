...
namespace Fourty6et2.ThirdParty.GlobalMouseKeyHook.HotKeys
{
    ...
    public class HotKeySet
    {
        ...
        {
            HotKeySet hotKeySet = obj as HotKeySet;
            if (hotKeySet == null)
                return false;

            int thisSize = 0;
            foreach (var keys in this.HotKeys)
                thisSize++;

            int thatSize = 0;
            foreach (var keys in hotKeySet.HotKeys)
                thatSize++;

            if (thisSize != thatSize)
                return false;

            foreach (var thisKeys in this.HotKeys)
            {
                bool exists = false;
                foreach (var thatKeys in hotKeySet.HotKeys)
